...
async def get_current_user(
    ...
    try:
        ...
        user_id = verify_token(token, "user")
        if user_id is None:
...
async def get_current_session(
    ...
    try:
        ...
        session_id = verify_token(token, "session")
        if session_id is None:
            logger.error("session_id_not_found")
            raise HTTPException(
                status_code=401,
                detail="Invalid authentication credentials",
                headers={"WWW-Authenticate": "Bearer"},
...
async def create_session(request: Request, user: User = Depends(get_current_user)):
    ...
    try:
        ...
        token = create_access_token(session_id, "session")
...
async def update_session_name(
    ...
    try:
        ...
        token = create_access_token(sanitized_session_id, "session")
    ...
    try:
        ...
        return [
            SessionResponse(
                ...
                token=create_access_token(session.id, "session"),
            )

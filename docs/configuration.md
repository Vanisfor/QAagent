...
| --- | --- | --- | --- |
| `DEEPSEEK_API_KEY` | — | Yes | Official DeepSeek platform API key |
| `DEEPSEEK_BASE_URL` | `https://api.deepseek.com` | No | Official DeepSeek OpenAI-compatible endpoint |
| `DEEPSEEK_THINKING_ENABLED` | `false` | No | Enable DeepSeek thinking mode; keep disabled until reasoning-content tool-call round trips are verified |
...
| Variable | Default | Description |
| --- | --- | --- |
| `TRACING_ENABLED` | `true` | Enable local JSONL tracing |
| `TRACE_DIR` | `logs/traces` | Daily trace JSONL directory |
| `TRACE_FILE_RETENTION_DAYS` | `7` | Local trace retention |
| `TRACE_SLOW_SPAN_THRESHOLD_MS` | `2000` | Slow-span threshold |

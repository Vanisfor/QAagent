...
```mermaid
flowchart LR
    JSONL["evals/data/input.jsonl"] -->|"load explicit records"| Evaluator
    Metrics["Metric prompts\nevals/metrics/prompts/*.md"] --> Evaluator
    Evaluator -->|"LLM judge\nper trace × metric"| Score["pass / fail"]
    Score --> Report["JSON report\nevals/reports/"]
```

1. **Load records** — reads explicit `trace_id`, `input`, and `output` JSONL records
...
EVALUATION_API_KEY=...   # defaults to DEEPSEEK_API_KEY if not set
```
